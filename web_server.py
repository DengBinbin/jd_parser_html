    ...
    def post(self):
        ...
        if "jdfile" in self.request.files:
            ...
            for meta in file_metas:
                ...
                elif re.search(u"yjbys.com",source_text):
                    result = jdParser.parser(htmlContent=source_text, jdFrom="yjbys", type=type)
                elif re.search(u"yingjiesheng.com", source_text):
                    result = jdParser.parser(htmlContent=source_text, jdFrom="yingjiesheng", type=type)
                elif re.search(u"dajie.com", source_text):
                    result = jdParser.parser(htmlContent=source_text, jdFrom="dajie", type=type)
                elif re.search(u"xiaoyuan.zhaopin.com", source_text):
                    result = jdParser.parser(htmlContent=source_text, jdFrom="zhilian", type=type)
                else:
            ...
            #校招
            elif re.search("yjbys.com", source_text):
                result = jdParser.parser(url=source_text, jdFrom="yjbys", type=type)
            elif re.search(u"yingjiesheng.com", source_text):
                result = jdParser.parser(url=source_text, jdFrom="yingjiesheng", type=type)
            elif re.search(u"dajie.com", source_text):
                result = jdParser.parser(url=source_text, jdFrom="dajie", type=type)
            elif re.search(u"xiaoyuan.zhaopin.com", source_text):
                result = jdParser.parser(url=source_text, jdFrom="zhilian", type=type)
            else:
        ...
        if source_text.startswith("http"):
            ...
            #     source_text=re.sub('<script.*?>.*?</script>','',source_text)
                ...
                if "html_content" in result:
                    source_text = result["html_content"]
        ...
        logo = result["jdInc"]["incLogo"] if type == "xiaozhao"  else logo

        self.render("outputjd_co.html",source=url,result=json_str,logo =logo ,photo = photo)
...
if __name__ == "__main__":
    ...
    app = tornado.web.Application(
        ...
        static_path='/home/dengbinbin/jd_parser_html/jd_data/test_cos/',
        debug=True,
        autoescape=None,
        )
