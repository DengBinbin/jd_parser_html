...
class XzParserYJBYS(XzParserTop):
    """
    对应届毕业生网站html进行jd解析
    """
    def __init__(self):
    ...
    def preprocess(self, htmlContent, fname=None, url=None):
        ...
        """
        self.refresh()
        self.result["jdInc"]["jdFrom"] = u"yjbys"

        html = ""
        if url != None:
            html = urlopen(url).read()
            html = unicode(html, 'gb2312', errors='ignore')
        elif htmlContent:
            html = htmlContent
        elif fname:
        ...
        #获取网页url的目的是通过url判断JD是何种类型JD
        if not url:
            url =  self.soup.find("link").attrs["href"]
        #判断输入jd是何种类别
        if not self.jdType:
            if url.find("jobshow") != -1:
                self.jdType = "special"
            elif url.find("zhaopin")!=-1:
                self.jdType = "regular"
            else:
                self.jdType = "unknown"

        print "the input jd type is : {0}".format(self.jdType)
        #对于不同类型的JD，采用不同类型创建soup的方法
        if self.jdType=="regular":
            self.headsoup = self.soup.find("div","top-main")
            self.jdstr = self.soup.find("div",id="job_detail")
        elif self.jdType=="special":
            self.headsoup = self.soup.find("div", "mingqi-banner")
            self.job_tag = self.soup.find("div", "cont_div")
            self.jdstr = self.soup.find("div", "introduce")
            #判断正文中是否有表格存在
            if self.jdstr.find("table"):
                self.has_table = True
        else:
            raise Exception("only support /special and /regular")
    ...
    def judge_jobType(self):
        ...
        '''
        if self.job_tag:
            for key in self.job_tag.find_all("li"):
                if key.get_text().split(u"：")[0].find(u"招聘职位") != -1:
                    if key.get_text().split(u"：")[1].find(u"实习") != -1:
                        self.jobType = u"实习"
                    else:
                        self.jobType = u"全职"
    ...
    def  regular_incname(self):
        if self.headsoup:
            incName = self.headsoup.find("div","company_info").find("span").get_text()
            self.result["jdInc"]["incName"] = incName

        incLogo = self.headsoup.find("a","company_pic").find("img").attrs["src"]
        #http://img.yjbys.com/2013_04/welcome.png是系统logo缺失的时候的默认图片，不需要将解析
        if incLogo and incLogo!="http://img.yjbys.com/2013_04/welcome.png":
            self.result["jdInc"]["incLogo"] = incLogo


    def regular_baseinfo(self):
        find_jobName = self.jdstr.find("h2")
        if find_jobName:
            self.jobName = find_jobName.get_text().strip()

        #从网页中自带的表格中去提取一些字段
        table_info = self.jdstr.find("table").find_all("tr")
        if table_info:
            table_info = [item.get_text().strip().split('\n') for item in table_info if item.get_text()]
            for row in range(len(table_info)):
                for index in range(len(table_info[row])):
                    if re.search(u"招聘日期",table_info[row][index]):
                        date = table_info[row][index+1].split(u"至")
                        if len(date)==2:
                            self.jdJob[self.jobName]["pub_time"] = date[0]
                            self.jdJob[self.jobName]["jobEndTime"] = date[1]
                    elif re.search(u"工作地点",table_info[row][index]):
                        self.jdJob[self.jobName]["jobWorkCity"] = table_info[row][index+1]
                    elif re.search(u"职位类别", table_info[row][index]):
                        self.jdJob[self.jobName]["jobType"] = table_info[row][index + 1]
                    elif re.search(u"学历要求", table_info[row][index]):
                        self.jdJob[self.jobName]["jobDiploma"] = table_info[row][index + 1]
                    elif re.search(u"招聘人数", table_info[row][index]):
                        self.jdJob[self.jobName]["jobNum"] = table_info[row][index + 1]
                    elif re.search(u"工资待遇", table_info[row][index]):
                        self.jdJob[self.jobName]["jobSalary"] = table_info[row][index + 1]
    '''
    首先得到职位描述，然后从大段的职位描述中去抽取字段
    这其中最关键的函数是self.extra_info函数，这个函数在base.py文件中
    '''
    ...
    def special_incname(self):
        ...
        '''
        if self.headsoup:
            incName = self.headsoup.find("div","nameTop").find("h1").get_text()
            incAliasName = self.headsoup.find("div","name").find("h2")
            if incAliasName:
                incAliasName = incAliasName.get_text()
            self.result["jdInc"]["incName"] = incName
            self.result["jdInc"]["incAliasName"] = incAliasName
            incLogo = self.headsoup.find("div","logo")
            if incLogo:
                incLogo = incLogo.find("img").attrs["src"]
                if incLogo == "http://img.cnrencai.com/com_logo.png":
                    incLogo = ""
                self.result["jdInc"]["incLogo"] = incLogo
    ...
    def special_incintro(self):
        ...
        '''
        if self.jdstr:
            self.intro = self.jdstr.find("div", "content").find_all("p")
            #首先找到整份JD如何划分
            self.find_title(self.intro)

            #调整输入格式，将所有英文冒号换成中文冒号换成中文冒号方便之后进行匹配，如果某些字段，例如URL需要英文冒号，
            #后面还需要换回英文冒号
            self.intro = [re.sub(u":",u"：",line.get_text().strip()) for line in self.intro]


            # 用CNN尝试去抽取，可能是训练数据不足，也可能是公司描述和职位描述相似度较高导致。
            ...
            for mode in modes:
                ...
                #     else:
                #         self.intro_range.append(-1)
                #         self.intro_range.append(-1)
                #     break
            if not self.result["jdInc"]["incIntro"]:
                for idx,line in  enumerate(self.jdstr.find_all("p")):
        ...
        if not self.has_table:
            ...
            if len(self.jobNameList) == 0:
                if self.job_tag:
                    for key in self.job_tag.find_all("li"):
                        if key.get_text().split(u"：")[0].find(u"招聘职位") != -1:
                            if key.get_text().split(u"：")[1].find(u"2016招聘")!=-1 or key.get_text().split(u"：")[1].find(u"2016校园招聘")!=-1 or key.get_text().split(u"：")[1].find(u"2016暑期")!=-1:
                                self.jobNameList.append(re.sub(u"2016(校园)?招聘|2016暑期实习生招聘",u"",key.get_text().split(u"：")[1]))
            ...
            #对职位名进行清洗，主要处理一行出现多个职位名以及职位名中存在的一些无关信息
            ...
            for jobName,line in zip(jobNameCopy,jobNameLineCopy):
                ...
                for eachName in self.PUNCTION.split(jobName):
                    ...
                    if len(eachName)!=0 and eachName[-1].islower():
                        continue
                    if  2<len(eachName)<13 and eachName[-4:-1] in self.position_postfix :
        ...
        # 表格分两种，一种是规整的m*n类型表格，另外一种是不规则的
           ...
           #因为表格的开头可能会有差异，因此当无法匹配到表格开头的时候需要修改此处的规则
            for idx, key in enumerate(titleList):
                print key
                if re.search(u"招聘岗位|实习生招聘|职位名称|招聘职位|实习岗位|岗位方向|定向岗位|岗位名称",key):
                    jobNameIdx = idx
    ...
    def special_contact(self):
        ...
        for line in self.intro:
            if line:
                ...
                if re.search(u'联络方式|微信 ：|联络电话|联系电话 ：|电话 ：|联系方式 ：|传真 ：|Tel ：', re.sub(u'：', u' ： ',re.sub(u':', u'：', line))) \
                    ...
                    print "url"
                    self.result["jdInc"]["incUrl"] = re.sub(u"：",u":",self.INC_URL.search(line).group(2))


    def other_info(self):
        '''

        判断是否还有信息没解析到可以利用到job_tag中的信息。

        '''
        if self.job_tag:
            for key in self.job_tag.find_all("li"):
                if key.get_text().split(u"：")[0].find(u"工作地区")!=-1:
                    workCity =  key.get_text().split(u"：")[1]
                elif key.get_text().split(u"：")[0].find(u"发布时间")!=-1:
                    pubTime = key.get_text().split(u"：")[1]

        #找到其他的一些信息,首先判断一些key是否缺少
        for jobName in self.jobNameList:
            if not self.check(jobName,"jobWorkCity"):
                self.jdJob[jobName]["jobWorkCity"] = workCity.strip()
            if not self.check(jobName,"pub_time"):
                self.jdJob[jobName]["pub_time"] = pubTime

        # if not self.jdJob[self.jobNameList[0]].has_key("email"):
        #     #再遍历一次去对每个职位进行判断
        #     for num in range(len(self.intro)):
        #         self.flag = None
        #         line = self.intro[num]
        #         if re.search(u"投递地址",line):
        #             if line.count(u"：")==1:
        #                 for jobName in self.jobNameList:
    ...
    def parser_xz(self, htmlContent=None, fname=None, url=None):
        ...
        if self.jdType=="regular":
            ...
            self.regular_baseinfo()
            self.regular_jobdesc()
        elif self.jdType=="special":
...
if __name__ == "__main__":
    ...
    # result = test.parser_xz(url="http://www.yjbys.com/special/jobshow47689.html")

    result = test.parser_xz(url="http://www.yjbys.com/mingqi/jobshow47744.html")
    print json.dumps(result, ensure_ascii=False, indent=4)
