...
class XzParserYingJieSheng(XzParserTop):
    """
    对应届毕生求职网html进行jd解析
    """
    ...
    def preprocess(self, htmlContent, fname=None, url=None):
        ...
        """
        self.refresh()
        self.result["jdInc"]["jdFrom"] = u"yingjiesheng"
        ...
        if not url:
            if self.soup.find("div","local") and self.soup.find("div","local").find("div","loca"):
                url = self.soup.find("div","local").find("div","loca").find_all("a")[-1].attrs["href"]
        # 判断输入jd是何种类别
        if not self.jdType:
            if url.find("job_") != -1:
                self.jdType = "regular"
        ...
        if self.jdType == "regular":
            self.section = self.soup.find("div", "main").find_all("div","section")
            if len(self.section)!=4:
                raise Exception("input jd wrong")
        elif self.jdType == "special":
            self.jdstr = self.soup.find("div", "main")
            title = self.jdstr.find("h1").get_text()
            if re.search(u"大学|事业单位|机关|政府|教师|局$|老师|医院",title):
                self.jdType="unknown"
            # 判断正文中是否有表格存在
    ...
    def find_title(self, list):
        ...
        if len(list.find_all("p"))==0:
            list = list.get_text()
            list = list.split('\n')
            list = [item.strip() for item in list if item.strip()]
            for idx in range(len(list)):
                if self.FIRST.search(list[idx]):
                    self.first.append(idx)
                elif self.THIRD.search(list[idx]):
                    self.third.append(idx)

        elif len(list.find_all("p"))>0:
            list = list.find_all("p")
            list = [item for item in list if item.get_text().strip()]
            print "dsdsadsad"
            # for idx in range(len(list)):
            ...
            #         self.third.append(idx)
            if self.jdstr.find("strong"):
                for idx in range(len(list)):
                    if self.FIRST.search(list[idx].get_text()):
                        self.first.append(idx)
                    if list[idx].find("strong"):
                        self.second.append(idx)
            else:
                for idx in range(len(list)):
                    if self.THIRD.search(list[idx].get_text()):
                        self.third.append(idx)
            list = [item.get_text().strip() for item in list if item.get_text().strip()]
        return list
    ...
    def judge_jobType(self):
        if self.job_tag:
            if self.job_tag.find(u"实习") != -1:
                self.jobType = u"实习"
            else:
                self.jobType = u"全职"

    '''
       html标签中分为四个section标签，其中0、1、3分别对应一些信息，最后再从职位描述中更细分去抽取信息
    '''
    def regular_incname(self):
        if len(self.section)>0:
            head = self.section[0]
        if head.find("h1"):
            self.result["jdInc"]["incName"] = head.find("h1").find("a").get_text()
        if head.find("ul","company_detail clearfix"):
            li =  head.find("ul","company_detail").find_all("li")
            for item in li:
                key  = item.find("strong").get_text()
                value = item.find("span").get_text()
                if key.find(u"所属行业")!=-1:
                    self.result["jdInc"]["incIndustry"] = value
                elif key.find(u"企业规模") != -1:
                    self.result["jdInc"]["incScale"] = value
                elif key.find(u"企业性质") != -1:
    ...
    def regular_jobname(self):
        ...
        find_jobName = info.find("h2")
        if find_jobName:
            self.jobName = find_jobName.find("a").get_text().strip()

        # 从网页中自带的表格中去提取一些字段
        job_info = info.find("div","job_list")
        if job_info.find("ul"):
            base_info = job_info.find("ul").find_all("li")
            for item in base_info:
                if item.find(u"：")==1:
                    continue
                key = item.get_text().split(u"：")[0]
                value = item.get_text().split(u"：")[1]
                if key ==u"工作地点":
                    self.jdJob[self.jobName]["jobCity"] = value
                elif key==u"有效日期":
                    if value.find(u"至")!=-1:
                        start_time = value.split(u"至")[0].strip()
                        end_time = value.split(u"至")[1].strip()
                        self.jdJob[self.jobName]["pub_time"] = start_time
                        self.jdJob[self.jobName]["jobEndTime"] = end_time
                elif key == u"招聘人数":
                    self.jdJob[self.jobName]["jobNum"] = value
                elif key==u"职位性质":
                    self.jdJob[self.jobName]["jobType"] = value

    def regular_jobdesc(self):
        info = self.section[1]
        job_desc = info.find("div", "j_i").get_text()
        self.jdJob[self.jobName]["jobDesc"] = job_desc
        job_desc = job_desc.split(u"\n")
        self.flag = None
        for idx,line in enumerate(job_desc):
            if line.strip():
                self.extra_info(line,idx,add_desc=False)


    def regular_incdesc(self):
        if len(self.section) >3:
            inc_info = self.section[3]
        if inc_info.find("img"):
            incLogo = inc_info.find("img").attrs["src"]
            incLogo  = "http://my.yingjiesheng.com"+incLogo
            self.result["jdInc"]["incLogo"] = incLogo


        if len(inc_info.find_all("p"))==2:
            incIntro = inc_info.find_all("p")[1].get_text()
        else:
            incIntro = inc_info.find("p").get_text()

        self.result["jdInc"]["incIntro"] = incIntro

        if inc_info.find("ul",id="contact"):
            contactInfo =inc_info.find("ul",id="contact").find_all("li")
            for item in contactInfo:
                if item.get_text().find(u"：") == -1:
                    continue
                key = item.get_text().split(u"：")[0]
                value = item.get_text().split(u"：")[1]
                print key
                if re.search(u"企业网址",key):
                    self.result["jdInc"]["incUrl"] = value
                if re.search(u"联系地址", key):
                    self.result["jdInc"]["incLocation"] = value
    ...
    def special_incname(self):
        ...
        '''
        find_inc = self.jdstr.find("h1")
        if find_inc:
            incName = find_inc.get_text().strip()

        if incName.find(u"实习生")!=-1:
            self.jobType=u"实习生"
        else:
            self.jobType=u"全职"


        if re.search(u"\[.*?\]",incName):
            incName = incName.split(u"]")[1]
            incName = re.split(u"2016招聘|招聘",incName)[0]
    ...
    def special_incintro(self):
        ...
        '''
        if self.jdstr:
            self.intro = self.jdstr.find("div", "jobIntro")
            # 首先找到切分点

            self.intro = self.find_title(self.intro)

            # 调整输入格式
            self.intro = [re.sub(u":", u"：", line) for line in self.intro]
            ...
            # 首先判断是否是bbs格式，因为bbs格式中会包含三行额外的信息
            if self.intro[0].find(u"发信人") != -1:
                is_bbs = True
            else:
                is_bbs = False
            ...
            if is_bbs:
                start_line = 2
                self.job_tag =self.intro[1].split(u"：")[1] if len(self.intro[1].split(u"："))==2 else self.intro[1]
            else:
                start_line = 0
            print u"是否为bbs格式:{0}".format(is_bbs)
            ...
            for mode in modes:
                ...
                if (mode and mode[0] > start_line):
                    if not is_bbs:
                        ...
                        for i in range(3,mode[0]):
                            self.result["jdInc"]["incIntro"] += self.intro[i]
                        self.intro_range.append(3)
                        self.intro_range.append(mode[0] - 1)
                        break
                # elif (mode and mode[1] > start_line):
            ...
            if not self.result["jdInc"]["incIntro"]:
                if len(self.jdstr.find_all("p"))>0:
    ...
    def special_jobname(self):
        ...
        if not self.has_table:
            ...
            # 3、还是没找到职位名，去招聘概况中提取,100只是为了提示职位名是在标题中找到的
            if len(self.jobNameList) == 0:
                if self.job_tag:
                    for eachName in re.split(self.PUNCTION,self.job_tag):
                        if self.JOBNAME.search(eachName):
                            self.jobNameList.append(eachName)
                            self.jobNameLine.append(100)
                    if len(self.jobNameList) == 0:
                        self.jobNameList.append(self.job_tag)
                        self.jobNameLine.append(100)
...
if __name__ == "__main__":
    ...
    result = test.parser_xz(url="http://www.yingjiesheng.com/job-002-315-509.html")
    print json.dumps(result, ensure_ascii=False, indent=4)
